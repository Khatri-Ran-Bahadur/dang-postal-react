export default function BikramSambatConverter() {
  this.bs = {
  ...
  this.ConvertInNepaliYMD = function(date) {
    var rn_date = date.split("-");
    this.nep_date = {};
    this.nep_date.year = this.to_nepali_number(rn_date[0]); //
    this.nep_date.month = this.to_nepali_number(rn_date[1]); //
    this.nep_date.date = this.to_nepali_number(rn_date[2]);
    this.nep_date.day = this.get_day_of_week(rn_date[2]);
    this.nep_date.nmonth = this.get_nepali_month(rn_date[1]);
    return this.nep_date;
  }

  this.AD_to_BS_ENG = function(yy, mm, dd) {
    yy = parseInt(yy);
    mm = parseInt(mm);
    dd = parseInt(dd);
    if (this.is_range_eng(yy, mm, dd) == false) {
      ...
      let def_eyy = 1944; //spear head english date...
      let def_nyy = 2000;
      let def_nmm = 9;
      ...
      let total_nDays = 0;
      let a = 0;
      let day = 7 - 1; //all the initializations...
      var m = 0;
      var y = 0;
      var i = 0;
      var j = 0;
      ...
      for (i = 0; i < yy - def_eyy; i++) {
        ...
        if (this.is_leap_year(def_eyy + i) == 1)
          for (j = 0; j < 12; j++) total_eDays += lmonth[j];
        else
          for (j = 0; j < 12; j++) total_eDays += month[j];
      }

      // count total no. of days in-terms of month
      for (i = 0; i < mm - 1; i++) {
        if (this.is_leap_year(yy) == 1) total_eDays += lmonth[i];
  ...
  this.convertNepToEng = function(date) {
    var rn_date = date.split("-");
    var nep_to_eng = this.nep_to_eng(rn_date[0], rn_date[1], rn_date[2]);
    var complete_date = nep_to_eng['year'] + '-' + nep_to_eng['month'] + '-' + nep_to_eng['date'];
